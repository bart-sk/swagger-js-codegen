...
function filterSwaggerPathsByMethods(swagger, methodsToFilter) {
  if (methodsToFilter === '*' || methodsToFilter === '') {
    return Object.assign({}, swagger, {
      paths: {},
    });
  }
  ...
  const operationIds = Object.keys(swagger.paths)
    .reduce((opids, path) => {
      const filteredOpIds = Object.keys(swagger.paths[path])
        .map((method) => {
          const operationId = swagger.paths[path][method].operationId;
          const methodsArray = methodsToFilter.split(',').map((m) => m.trim());
          if (operationId && methodsArray.includes(operationId)) {
            return swagger.paths[path][method].operationId;
          }
          return null;
        })
  ...
  const newPaths = Object.keys(swagger.paths).reduce((finalPaths, path) => {
    ...
    if (Object.keys(filteredDef).length > 0) {
      return Object.assign(finalPaths, { [path]: filteredDef });
    }
...
var resolveMethodReturnType = function (type) {
  if (type === 'schema' || (type && type.includes('default'))) {
    return 'any';
  }
  return type;
};
